...
class DB
{
	...
	{
		try {
			$this->koneksi = new PDO("mysql:host=localhost;dbname=beasiswa", "root", "", array(PDO::ATTR_PERSISTENT => TRUE));
...
class User extends DB
{
	...
	function __construct()
	{
		$this->sqlRegister = $this->bukaKoneksi()->prepare("insert into users (nama_lengkap, username, password, email, kelas) values (:nama_lengkap, :username, :password, :email, :kelas)");
	...
	function Register($nama_lengkap, $username, $password, $email, $kelas)
	{
		try {
			...
			$this->sqlRegister->bindParam(':kelas', $kelas);
...
class guru extends DB
{
	function GetData($qry_custom)
	{
		try {
			$sql = $this->bukaKoneksi()->prepare("select * from guru " . $qry_custom);
			$sql->execute();
			return $sql;
		} catch (PDOException $e) {
			print $e->getMessage();
...
class kelas extends DB
{
	...
	{
		try {
			$sql = $this->bukaKoneksi()->prepare("select * from kelas " . $qry_custom);
			$sql->execute();
			return $sql;
		} catch (PDOException $e) {
			print $e->getMessage();
